...
const ratingStyles: Record<string, string> = {
  Excellent: "border-emerald-400/40 bg-emerald-400/10 text-emerald-100 ring-emerald-400/30",
  ...
  "High Risk": "border-rose-400/40 bg-rose-400/10 text-rose-100 ring-rose-400/30",
  Unrated: "border-slate-400/40 bg-slate-400/10 text-slate-100 ring-slate-400/30",
};
      ...
      <div className="grid gap-6 lg:grid-cols-[1fr_18rem] lg:items-start">
        <div>
          ...
          <p className="mt-4 max-w-3xl text-sm leading-6 text-slate-400">
            Scores run from 0 to 100. A higher score means the uploaded report showed
            stronger profitability, financial health, and cash flow signals.
          </p>
        </div>
  ...
  return (
    <article className="rounded-3xl border border-slate-800 bg-slate-900/80 p-6 shadow-xl">
      <div className="flex flex-col gap-2 sm:flex-row sm:items-end sm:justify-between">
        ...
        <p className="text-sm text-slate-400">Each pillar is scored from 0 to 100.</p>
      </div>
        ...
        {pillars.map((pillar) => (
          ...
          >
            <div className="flex items-start justify-between gap-4">
              ...
              <span className="rounded-full bg-cyan-400/10 px-3 py-1 text-sm font-semibold text-cyan-200 ring-1 ring-cyan-400/20">
...
export function ExtractionSummaryBox({ text }: TextBoxProps) {
  return (
    <article className="rounded-3xl border border-violet-400/20 bg-violet-400/10 p-6 shadow-xl ring-1 ring-violet-400/10">
      <p className="text-sm font-semibold uppercase tracking-[0.25em] text-violet-200">
        AI extraction summary
      </p>
      <h2 className="mt-2 text-xl font-semibold text-white">What Gemini found</h2>
...
export function DisclaimerBox({ text }: TextBoxProps) {
  return (
    <aside className="rounded-3xl border border-amber-400/20 bg-amber-400/10 p-6 shadow-xl ring-1 ring-amber-400/10">
      <p className="text-sm font-semibold uppercase tracking-[0.25em] text-amber-200">
        Disclaimer
      </p>
      <p className="mt-3 leading-7 text-slate-200">
        {text ||
          "This dashboard is for learning only. It does not tell you what to do with any investment."}
      </p>
    </aside>
  );
}
...
export function PrivacyNoteBox({ text }: TextBoxProps) {
  return (
    <aside className="rounded-3xl border border-cyan-400/20 bg-cyan-400/10 p-6 shadow-xl ring-1 ring-cyan-400/10">
      <p className="text-sm font-semibold uppercase tracking-[0.25em] text-cyan-200">
        Privacy note
      </p>
      <p className="mt-3 leading-7 text-slate-200">
        {text ||
          "Uploaded files are used for the current analysis session only. No privacy note was returned by the backend."}
      </p>
    </aside>
  );
}
